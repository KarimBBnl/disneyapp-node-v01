...
async function fetchMovies() {
    ...
    try {
        const response = await fetch(`${API_URL}?type=series`);
...
function renderMovies(movies) {
    ...
    if (movies.length === 0) {
        grid.innerHTML = '<p class="error-msg">No se encontraron series para este criterio.</p>';
        return;
    }
